    ...
    <div>
        ...
        <v-list three-line>
            <template v-for="(test, index) in reportState.reports">
                ...
                >
                    <v-list-tile-avatar> {{ (test.passed / test.total) * 100 }}% </v-list-tile-avatar>
                    <v-list-tile-content>
                        <v-list-tile-title>
                            <span>{{ formatDate(test.startTime) }}</span>
                            <span style="float: right;" v-if="test.tags && test.tags.length > 0"
                                >Tags: {{ test.tags.toString() }}</span
                            >
                        </v-list-tile-title>
                        <v-list-tile-sub-title>
                            Total: {{ test.total + test.skipped }}, Passed: {{ test.passed }}, Failed: {{ test.total - test.passed }},
                            Skipped: {{ test.skipped }} Time: {{ execTime(test) }}
                        </v-list-tile-sub-title>
                    </v-list-tile-content>
                </v-list-tile>
